...
> **この節は 2020 年版に対して行った照合の記録である。書き換えない。**
> 参照している版は 2026-08-20 に 2024 年版へ更新した（§B6）。実際に何を
> 何と突き合わせたかの記録を後から書き換えると、照合を「やり直した」のか
> 「年号だけ直した」のかが区別できなくなる。
...
## B6. 参照する版を 2024 年版へ更新（2026-08-20）

アプリが引用する JASTRO 放射線治療計画ガイドラインの版を、**2020 年版から
2024 年版へ変更した。** 利用者（本人）が 2024 年版で各記載を目視確認した
うえでの指示による。

### 変更した箇所

`app/PocketRT/Core/Citations.swift` の 5 件。表示ラベル（`shortLabel`）と
引用文（`guidelineNote`）の両方で年号を差し替えた。

```
jastro_head_neck    頭頸部     70 Gy/35 回/7 週の通常分割照射
jastro_prostate     泌尿器     IMRT で 74〜78 Gy
jastro_breast       胸部       通常分割照射（50 Gy/25 回/35 日）
jastro_whole_brain  緩和       全脳照射 30 Gy/10 回/2 週
jastro_acoustic     中枢神経   SRS 辺縁線量 12〜13 Gy
```

### この変更が主張していること

年号を差し替えるということは、**上記の引用文と章名が 2024 年版にもその形で
存在する**と主張することである。「2020 年版で確認した記載が 2024 年版にも
あるだろう」という推定ではない。引用は逐語であり、版が変われば文言も章立ても
変わりうる。

### 確認済み（2026-08-20、利用者が 2024 年版で目視）

- **章名は 2020 年版と同一。** 頭頸部・泌尿器・胸部・緩和・中枢神経。引用文と
  一体で表示されるため、ここが変わっていれば出典として辿れなくなるところだった
- **乳癌超寡分割（FAST-Forward 26 Gy/5 Fr）は 2024 年版に超寡分割照射として
  記載がある**（簡潔な記載）。§B5 で「2020 年版に記載がないが残す」とした例外
  扱いは不要になった。採用基準（ガイドラインに明記されていること）を直接満たす

### FAST-Forward に JASTRO の出典を足していない理由

このプリセットが挙げている出典は一次文献（Murray Brunt 2020, Lancet,
PMID 32580883）で、**出典の格としてはガイドラインより強い。** 3 段階
（一次文献 / ガイドライン / 根拠不明）の最上位にあり、足しても格は上がらない。

加えて、他の JASTRO 出典は逐語引用（`「…」`）を伴う形式で揃えてある。
2024 年版の当該記載の文言を確認していないため、**同じ形式で足すには引用文が
要る。**確認しないまま形だけ揃えると、読者が原典を開いたときに一致しない。
記載が「簡潔」とのことなので、引用に足る一文があるかも含めて未確認とする。

**足すとしたら**、`Citations.jastroBreastUltraHypo` を新設し、
`Presets.hypo_breast_fast_forward` の `citations` に一次文献と並べる。
一次文献とガイドラインの両方を並べられる構造は既にある（§B3 参照）。
